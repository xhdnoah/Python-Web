...
class BaseDB:
    ...
    def connect(self):
        # 返回一个数据库连接对象
        return pymysql.connect(host=self.host, user=self.user, port=self.port, passwd=self.password, db=self.database,
    ...
    # SQL 语句执行方法，数据操作，增，删，改，查
    @DBResult.handler
    def execute(self, sql, params=None):
        # 获取数据库连接对象上下文
        with self.conn as cursor:
            # 如果参数不为空并且时 Dict 类型时，把 SQL 语句与参数一起传入 execute 中调用，反之直接调用 execute
            # 执行语句并获取影响条目数量
            rows = cursor.execute(sql, params) if params and isinstance(
                params, dict) else cursor.execute(sql)
    ...
    # 在实际开发中，有些应用场景需要对刚插入的数据再次操作，为方便操作就需要拿到这条数据的标识
